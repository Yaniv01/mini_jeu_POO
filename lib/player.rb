...
class HumanPlayer < Player #initialisation ce la class HumanPlayer
	attr_accessor :weapon_level
 
	def initialize(name) # méthode initialize avec modif des hp et ajouts d'une instance
		@name= name #on pouvait aussi utiliser super mais ici il n'y avait qu'une ligne
		@life_points = 100
		@weapon_level = 1
	...
	def show_state #méthode qui montre l'état du joueur avec rajout de son arme et de son niveau
		puts "#{name} a #{@life_points} points de vie et une arme de niveau #{@weapon_level}"

	end #fin méthode show_state

	def compute_damage #méthode qui renvoie les dégats en fonction du niveau de l'arme
	rand(1..6)*weapon_level

	end # fin de la méthode compute_damage
	
	def search_weapon #méthode qui permet au joueur de trouver une nouvellle arme 
	...
	def search_health_pack #méthode de recherche du pack de points de vie

		health_pack=rand(1..6)
		if health_pack == 1 #condition d'application du heal
